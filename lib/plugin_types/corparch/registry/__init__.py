...
class RegistryConf:
    ...
    async def save(self, cursor):
        ...
        created_rt = await self._backend.save_registry_table(
            cursor, self._corpus_id, variant=self._variant,
            values=[(x.name, x.value) for x in self.simple_items])
        # now we fill in self references MAPTO, FROMATTR
        for pos in self.posattrs:
            fromattr_id = None
            mapto_id = None
            await self._backend.update_corpus_posattr_references(cursor, self._corpus_id, pos.name, fromattr_id, mapto_id)
        ...
        if created_rt:
            ...
            for i, struct in enumerate(self.structs):
                await self._backend.save_corpus_structure(
                    cursor, corpus_id=self._corpus_id, name=struct.name, position=i, values=[
                ...
                for i, structattr in enumerate(struct.attributes):
                    await self._backend.save_corpus_structattr(
                        cursor, self._corpus_id, struct.name, structattr.name,
                        i, [(x.name, x.value) for x in structattr.attrs])

            for i, sc in enumerate(self.subcorpattrs):
                struct, attr = sc.split('.')
                await self._backend.save_subcorpattr(cursor, self._corpus_id, struct, attr, i)

            for i, fc in enumerate(self.freqttattrs):
                struct, attr = sc.split('.')
                await self._backend.save_freqttattr(cursor, self._corpus_id, struct, attr, i)
